...
async function startCampaign(chatId, message, contacts) {
    ...
    for (let i = 0; i < contacts.length; i++) {
        ...
        try {
            ...
            if ((i + 1) % 10 === 0 || i === contacts.length - 1) {
                const progress = `📊 Progress: ${state.currentCampaign.sent}/${state.currentCampaign.total}\n` +
                               `✅ Sent: ${state.currentCampaign.sent}\n` +
                               `❌ Failed: ${state.currentCampaign.failed}`;
                await telegram.sendMessage(chatId, progress);
            }
        ...
        if (batchCount >= BATCH_SIZE && i < contacts.length - 1) {
            console.log('Taking batch break...');
            await telegram.sendMessage(chatId, '⏸️ Taking a 1-minute break to avoid detection...');
    ...
    const summary = `✅ *Campaign Completed!*\n\n` +
                   ...
                   `❌ Failed: ${state.currentCampaign.failed}\n` +
                   `⏱️ Duration: ${Math.round((state.currentCampaign.endTime - state.currentCampaign.startTime) / 1000 / 60)} minutes`;
